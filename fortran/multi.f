...
!****************************************************************************
!
!  PURPOSE: CALCULATION OF POWER SPECTRUM BY TWO FIELD INFLATION
!
!  STATUS :IN PROGRESS
!
!****************************************************************************
      module global 
      implicit none
      integer, parameter :: nf=2!1!, nf2=4
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      real*8 yf_r2(nf),yl_r2(nf),dyf_r2(nf),dyl_r2(nf)
      real*8 yf_i2(nf),yl_i2(nf),dyf_i2(nf),dyl_i2(nf)
      real*8 yb_r2(1),dyb_r2(1),yb_i2(1),dyb_i2(1)
      real*8 zeta_r2,zeta_i2,isocurv_r2, isocurv_i2
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!    
...
!     Read the initial conditions for the fields    
      ...
      ! read(11,*) x(i)!, xd(i)
       ...
       x(1)=7.d0!12!16.d0!0.517d0!6.33!18.d0!
       x(2)=7.31d0!12!0.96d0!3.96d-3!0.001d0!
       call xic(x,xd)
      print*, xd(i)
      !read(*,*)
      enddo
...
!     Unit "bardeen_initial.txt"        
     ...
     + xx,xxd,yl0_i,dyl0_i,ks,ps0_i,dps0_i) 
      ! ATTENTION !!! PROSOXI!!! DEN EXO ALLAKSEI TA BARDEEN!!!
...
!     Solution of linear perturbations+fields+Bardeen
      ...
      DO k=2,timestep2,k_step!2200,2200!
...
!     Imafinary part:
      ...
      r(j_bardeen+1+nf)=dyl0_i(1,it)
      r(j_bardeen+2+nf)=0
!      Real part for the second field:
       p2(i)=xx(i,it)
      p2(1+nf)=0
      p2(2+nf)=yl0_r(2,it)
      p2(j_bardeen+i)=xxd(i,it)
      p2(j_bardeen+1+nf)=0
      p2(j_bardeen+2+nf)=dyl0_r(2,it)
      
!      Imaginary part for the second field:      
      r2(i)=xx(i,it)
      r2(1+nf)=0
      r2(2+nf)=yl0_i(2,it)
      r2(j_bardeen+i)=xxd(i,it)
      r2(j_bardeen+1+nf)=0
      r2(j_bardeen+2+nf)=dyl0_i(2,it)
      enddo
      !print*,  dyl0_r(2,it),dyl0_i(2,it)
...
!     Imafinary part:
      ...
      yf_r2(i) =p2(i)
      yl_r2(i) =p2(i+nf)
      dyf_r2(i)=p2(j_bardeen+i)
      dyl_r2(i)=p2(j_bardeen+i+nf)
!     Imafinary part:
      yf_i2(i) =r2(i)
      yl_i2(i) =r2(i+nf)
      dyf_i2(i)=r2(j_bardeen+i)
      dyl_i2(i)=r2(j_bardeen+i+nf)
      enddo
        !print*, dyl_r2(2),dyl_i2(2)
        !read(*,*)
...
!     Imafinary part:
      ...
      !     Real part:
      p2(j_bardeen) =ps0_r(it)
      p2(j_dbardeen)=dps0_r(it)
      yb_r2(1) =p2(j_bardeen)
      dyb_r2(1)=p2(j_dbardeen)
!     Imafinary part:
      r2(j_bardeen) =ps0_i(it)
      r2(j_dbardeen)=dps0_i(it)
      yb_i2(1) =r2(j_bardeen)
      dyb_i2(1)=r2(j_dbardeen)
      !print*, yb_r2(1), yb_i2(1)
      !read(*,*)
 !     print*,"kmode=",kmode
...
!     Imafinary part:
      ...
      zeta_r2=zeta(yf_r2,dyf_r2,yl_r2,yb_r2(1))
      zeta_i2=zeta(yf_i2,dyf_i2,yl_i2,yb_i2(1))
       !print*, zeta_r2, zeta_i2
      pr(it)=kmode**3.d0*(zeta_r**2.d0+zeta_i**2.d0
     ++zeta_r2**2.d0+zeta_i2**2.d0)/(2.d0*pi**2.d0)  
       
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!11
      isocurv_r=isocurv(yf_r,dyf_r,yl_r,yb_r(1))
      isocurv_i=isocurv(yf_i,dyf_i,yl_i,yb_i(1))
      isocurv_r2=isocurv(yf_r2,dyf_r2,yl_r2,yb_r2(1))
      isocurv_i2=isocurv(yf_i2,dyf_i2,yl_i2,yb_i2(1))
      p_isocurv(it) =kmode**3.d0*(isocurv_r**2.d0+isocurv_i**2.d0
     ++isocurv_r2**2.d0+isocurv_i2**2.d0)
     +/(2.d0*pi**2.d0)!
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      !PRINT*, r2(1), p2(1)
      !read(*,*)
      ...
      !call rk4two(r,drdt,m,t(j),dt)
       ...
       call odeint(p2,m,t(j),t(j+1),epsi,dt)
       call odeint(r2,m,t(j),t(j+1),epsi,dt)
...
!     Imafinary part:
      ...
      zeta_i2=zeta(yf_i2,dyf_i2,yl_i2,yb_i2(1))
        !print*, zeta_r2, zeta_i2
      !read(*,*)!zeta_i2
      pr(j)=kmode**3.d0*(zeta_r**2.d0+zeta_i**2.d0 
     ++zeta_r2**2.d0+zeta_i2**2.d0)/(2.d0*pi**2.d0)  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                                        
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!11
     
       isocurv_r2=isocurv(yf_r2,dyf_r2,yl_r2,yb_r2(1))
      isocurv_i2=isocurv(yf_i2,dyf_i2,yl_i2,yb_i2(1))! PROSOXI DEN EINAI ETOIMO AKOMA!!!
      
      !print*, isocurv_r, isocurv_i,isocurv_r2!, isocurv_i2
      !read(*,*)
...
!      print*, t(it),pr(j-1),kmode
      ...
      !!ks(k)=kstar/(cq*H(1)*dexp(t(i_norm)))
       ...
       write(93,*) t(k) +n_back,prk(k),ks_norm!/1d14
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!      
      ...
      implicit none
       ...
       real*8 VV,x(nf)
!      VV=8.d-8*x(1)**2.d0
!      VV=2.9d-18*((-0.1d0+x(1))/300000.d0 -(-0.1d0 +x(1))**2.d0/100.d0 +
!     + 19999.999999999993d0*x(1)**2.d0*x(2)**2.d0 + (1.d0 -
!     + 99.99999999999999d0*x(2)**2.d0)**2.d0)
       real*8 m,d,b,c1,c2,cc,mm,lm,c4,a1,ll,c3,L
        real*8 mf,mx
        real*8 vinf,v0,cs,sf
      a1=1.d0
       m=2.484d-5 
      b=26.202586d0
      !4.397712
      ...
      l=1.00025d0
      c3=0
!       VV=             (3*(1 - dexp(Sqrt(2.d0/3.d0)*x(1)))**2
!     -*((c1*(1 - dexp(Sqrt(2.d0/3.d0)*x(1)))**2)/
!     -       dexp((b*(1 - dexp(Sqrt(2.d0/3.d0)*x(1)))**2)/
!     -(1 + dexp(Sqrt(2.d0/3.d0)*x(1)))**2) + 
!     -      (1 + dexp(Sqrt(2.d0/3.d0)*x(1)))**2)*
!     -((c1*(-1 + dexp(Sqrt(2.d0/3.d0)*x(1)))**2)/
!     -       dexp((b*(-1 + dexp(Sqrt(2.d0/3.d0)*x(1)))**2)
!     -/(1 + dexp(Sqrt(2.d0/3.d0)*x(1)))**2) + 
!     -      (1 + dexp(Sqrt(2.d0/3.d0)*x(1)))**2)*(1 - 
!     - dexp(Sqrt(2.d0/3.d0)*x(1)) + l +
!     - dexp(Sqrt(2.d0/3.d0)*x(1))*l)**2*m**2)/
!     -  (16.*dexp(2*Sqrt(2.d0/3.d0)*x(1))*(1 + 
!     -2*dexp(Sqrt(2.d0/3.d0)*x(1)) + 
!     -dexp(2*Sqrt(2.d0/3.d0)*x(1)))**2*l**2)
     
c      mf=1.395d-6**2.d0
c      mx=(7.d0*1.395d-6)**2.d0
c      VV=0.5d0*mf*x(1)**2.d0+0.5d0*mx*x(2)**2.d0  !double quatratic inflation
     
c      mf=1.2D0
c      mx=0.06**2.D0
c      VV=mf*(x(1)**2/2. + x(2)**2/(mx + x(2)**2))!braglia
     
!       VV=(5.37D-13)*x(1)**2*dexp(-0.05d0*x(2)**2)
     
!       Generating PBH and small-scale GWs in two-field models of inflation
        mx=1/500.d0
         mf=6.d0
        VV= x(1)**2/(mf + x(1)**2)+ mx*x(2)**2/2. !braglia
 
     
!      vv=8.62d-15*(1-0.0476*x(2)**2.d0)+ 
!     +8.62d-15*10*(1-dexp(-x(1)**2.d0/(0.155**2.d0)))
!       vinf=1.d0
!       v0=2.3d-12
!       cs=500
!       sf=5.477
!       vv=vinf*(v0/2)*x(2)**2.d0+vinf*cs*
!     +(1-dexp(-x(1)**2/sf**2.d0))
       
       
     
          ! VV=       (0.1*Tanh(x(1)/Sqrt(6.d0)) + 0.0007*
 !    -Tanh(100000*(-0.9811333867328498 + 
 !    -Tanh(x(1)/Sqrt(6.d0)))) + 0.0007*
 !    -Tanh(100000*(-0.9787014277530225 + Tanh(x(1)/Sqrt(6.d0)))) + 
 !    -    0.0007*Tanh(100000*(-0.9759597865125814 +
 !    - Tanh(x(1)/Sqrt(6.d0)))) + 0.0007*Tanh(100000*
 !    -(-0.972870068569409 + Tanh(x(1)/Sqrt(6.d0)))))**2
      end
...
!      ks(i)=kstar*(dexp(t(i)-t(1))*logosH(i))
      ...
      laa1=0.d0
      do a=2,2!1,nf
      laa1=laa1+abs(ll(a,a))
      enddo 
       laad1=0.d0
      do a=2,2!1,nf
      laad1=laad1+abs(lld(a,a,a))*xd(a)
      enddo
      ...
      dlx_i(1,i)=-temp*(ks(i)/(dexp(t(i))*H(i)))
            ...
            temp=dsqrt(1.d0/2.d0)/(dsqrt(laa1*ks(i)) *dexp(t(i))) 
      lx_r(2,i)=temp
      lx_i(2,i)=0.d0
      dlx_r(2,i)=-temp*(1.d0+laad1/(2.d0*laa1))
      dlx_i(2,i)=-temp*(ks(i)/(dexp(t(i))*H(i)))
      enddo
       enddo
...
!       isocurv=-xd(2)*xl(1)+xd(1)*xl(2)
       isocurv=-dexp(8.d0*x(1))*xd(2)*xl(1)+
     +dexp(8.d0*x(1))*xd(1)*xl(2)
       isocurv=isocurv/sd**2.d0    
      else
          isocurv=0.d0
      endif
...
!      lll=x(i)**2.d0*x(j)**3.d0
      ...
      lll=a
              ...
              lll=dexp(16.d0*x(1))!1.d0!
              !lll=1+(1.086/0.155)*x(1)!7.5d0*x(1)!
...
12    enddo
      ...
      do l=1,nf
       if(k.eq.l) then
       g=g+0.5d0*(ll_inv(k,l))*(lld(l,i,j)+lld(l,j,i)-lld(i,j,l))
...
11    continue
      ...
      integer i,n,j,io,iostat,ndata,k,n_gauleg
      parameter(n=10000,n_gauleg=1000)
...
c       theta=HeavisideTheta(x)
      ...
      ! implicit none
      ! integer k,timestep
      ! parameter(timestep=7000)
      ! real*8 s,d,ic,is,theta,fun,cg
      ! real*8 prk_plus(timestep),prk_min(timestep),pi
      ! parameter (cg=0.4d0)
      ! pi=dacos(-1.d0)
      !    IC= -36.d0*pi*theta(s-1)*(s**2.d0+d**2.d0-2.d0)**2.d0/
      !+(s**2.d0-d**2.d0)**3.d0
      ...
      !+ dlog(abs((d**2-1) /(s**2-1)))+2.d0)
      !  
      !  fun=(cg/72.d0)*(s**2.d0-(1.d0/3.d0))*(d**2.d0-(1.d0/3.d0))**2.d0
      !+*prk_plus(k)*prk_min(k)
      !+/((s**2.d0+d**2.d0)**2.d0)*(IC**2.d0+IS**2.d0)
      !
      ! end
